    ...
    <div className="space-y-4 p-4 animate-pulse">
      ...
      <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-lg w-fit">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="h-9 w-28 bg-slate-200/60 rounded-md" />
        ))}
      </div>
      ...
      <div className="flex gap-2 flex-wrap">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="h-9 w-40 bg-slate-100 rounded-lg" />
        ))}
      </div>
      {/* Table skeleton */}
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        {Array.from({ length: 8 }).map((_, i) => (
          <div key={i} className="px-4 py-3 border-b border-slate-50 flex items-center gap-4">
            ...
            <div className="h-3 w-28 bg-slate-100 rounded flex-1" />
