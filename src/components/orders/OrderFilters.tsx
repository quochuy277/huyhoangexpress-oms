  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
        ...
        <select value={currentCarrier} onChange={(e) => updateParam("carrier", e.target.value)} className="px-3 py-1.5 text-xs border border-slate-200 rounded-lg bg-white focus:ring-2 focus:ring-blue-500 text-slate-600">
          <option value="">Tất cả đối tác</option>
          {CARRIERS.map((c) => <option key={c} value={c}>{c}</option>)}
        </select>
        <input type="date" value={currentFromDate} onChange={(e) => updateParam("fromDate", e.target.value)} className="px-3 py-1.5 text-xs border border-slate-200 rounded-lg bg-white focus:ring-2 focus:ring-blue-500 text-slate-600" title="Từ ngày" />
        <span className="text-xs text-slate-400">→</span>
        <input type="date" value={currentToDate} onChange={(e) => updateParam("toDate", e.target.value)} className="px-3 py-1.5 text-xs border border-slate-200 rounded-lg bg-white focus:ring-2 focus:ring-blue-500 text-slate-600" title="Đến ngày" />
        ...
        {hasAnyFilters && (
          <button onClick={clearAll} className="flex items-center gap-1 px-2 py-1 text-xs text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors ml-auto">
      ...
      {showAdvanced && (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 pt-3 border-t border-slate-100 animate-in fade-in slide-in-from-top-2">
          ...
          <div>
            ...
            <select value={currentShopName} onChange={(e) => updateParam("shopName", e.target.value)} className="w-full px-2 py-1.5 text-xs border border-slate-200 rounded-lg bg-white focus:ring-2 focus:ring-blue-500 text-slate-700">
          ...
          {/* Has Notes */}
          <div>
            <label className="block text-[10px] font-semibold text-slate-500 uppercase mb-1">Ghi Chú</label>
            <select value={currentHasNotes} onChange={(e) => updateParam("hasNotes", e.target.value)} className="w-full px-2 py-1.5 text-xs border border-slate-200 rounded-lg bg-white focus:ring-2 focus:ring-blue-500 text-slate-700">
          ...
          {/* Sales Staff */}
          <div>
            <label className="block text-[10px] font-semibold text-slate-500 uppercase mb-1">NV Kinh Doanh</label>
            <select value={currentSalesStaff} onChange={(e) => updateParam("salesStaff", e.target.value)} className="w-full px-2 py-1.5 text-xs border border-slate-200 rounded-lg bg-white focus:ring-2 focus:ring-blue-500 text-slate-700">
          ...
          {/* Partial Order */}
          <div>
            <label className="block text-[10px] font-semibold text-slate-500 uppercase mb-1">Loại Đơn</label>
            <select value={currentPartialOrderType} onChange={(e) => updateParam("partialOrderType", e.target.value)} className="w-full px-2 py-1.5 text-xs border border-slate-200 rounded-lg bg-white focus:ring-2 focus:ring-blue-500 text-slate-700">
              <option value="">Tất cả</option>
          ...
          {/* Region Group */}
          <div>
            <label className="block text-[10px] font-semibold text-slate-500 uppercase mb-1">Vùng Miền</label>
            <select value={currentRegionGroup} onChange={(e) => updateParam("regionGroup", e.target.value)} className="w-full px-2 py-1.5 text-xs border border-slate-200 rounded-lg bg-white focus:ring-2 focus:ring-blue-500 text-slate-700">
